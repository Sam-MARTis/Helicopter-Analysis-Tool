...
from tkinter import ttk # I hate tkinter so so so so much
...
class Interface:
    def __init__(self, root):
        self.root = root
        self.root.title("Helicopter Simulator")
    ...
    def create_gui(self):
        ...
        self.mass_var = tk.DoubleVar(value=5090.0)
        
        mass_label_frame = ttk.Frame(control_frame)
        mass_label_frame.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Label(mass_label_frame, text="Mass (kg):").pack(side=tk.LEFT)
        ...
        self.rotor_radius_var = tk.DoubleVar(value=3.6)
        
        radius_label_frame = ttk.Frame(control_frame)
        radius_label_frame.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Label(radius_label_frame, text="Rotor Radius (m):").pack(side=tk.LEFT)
        ...
        self.root_chord_var = tk.DoubleVar(value=0.09)
        ...
        self.tip_chord_var = tk.DoubleVar(value=0.05)
        ...
        update_button = ttk.Button(right_frame, text="Update", command=self.update_simulation)
    ...
    def update_visualization(self):
        ...
        if hasattr(self, 'performance'):
            ...
            arrow_length = min(max_radius, abs(net_force) / arrow_scale)
