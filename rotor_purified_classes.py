# -*- coding: utf-8 -*-
"""rotor_purified_classes.py

Class-based implementation of rotor aerodynamics calculations.
Organized into Environment, Rotor, and Helicopter classes for better modularity.

Originally derived from rotor_aerodynamics_assignment.ipynb
...
class Helicopter:
    ...
    def mission_analysis(self, climb_velocities: List[float], n_divisions: int = 50) -> List[Dict[str, Union[float, bool]]]:
        if not self.rotor_set:
            raise ValueError("Rotor has not been set up.")
        
        results: List[Dict[str, Union[float, bool]]] = []
        for climb_vel in climb_velocities:
            performance: Dict[str, float] = self.calculate_hover_performance(climb_vel, n_divisions)
            thrust_required: float = self.calculate_thrust_required_for_hover()
            can_achieve: bool = performance['thrust'] >= thrust_required
            
            result: Dict[str, Union[float, bool]] = {
                'climb_velocity': climb_vel,
                'thrust_available': performance['thrust'],
                'thrust_required': thrust_required,
                'power_required': performance['power'],
                'torque': performance['torque'],
                'can_achieve': can_achieve,
                'excess_thrust': performance['thrust'] - thrust_required
            }
...
if __name__ == '__main__':
    def test_helicopter_analysis():
        print("=== Helicopter Analysis Tool Test ===\n")
        ...
        print(f"Environment set at altitude: {env.altitude} m")
        ...
        rotor.set_rotor_parameters(
            number_of_blades=4,
            omega=27.0,  # rad/s
            blade_mass=50.0,  # kg per blade
            NACA_for_airfoil="0012",
            radius_of_rotors=5.5,  # m
            root_cutout=0.5,  # m
            angle_of_attack=8.0,  # degrees
            root_chord=0.4,  # m
            tip_chord=0.2,  # m
            root_pitch=12.0,  # degrees
            slope_pitch=-0.08  # twist
        )
        print(f"Rotor configured: {rotor.number_of_blades} blades, radius {rotor.radius_of_rotors} m")
        
        print("\n3. Setting up Helicopter...")
        helicopter = Helicopter(env)
        helicopter.set_fuselage_parameters(
            mass=1200.0,  # kg
            length=12.0,  # m
            width=2.5,  # m
            height=3.0,  # m
        ...
        helicopter.set_engine_parameters(
            mass=300.0,  # kg
            BSFC=0.3,  # kg/kWh
            shaft_power_conversion_efficiency=0.92,
            fractional_power_tail_rotor=0.15
        )
        helicopter.set_rotor(rotor)
        
        print(f"Total helicopter mass: {helicopter.get_total_mass():.1f} kg")
        
        print("\n4. Testing atmospheric calculations...")
        temperature = env.get_temperature()
        pressure = env.get_pressure(temperature)
        density = env.get_density(temperature)
        print(f"At {env.altitude} m altitude:")
        ...
        print("\n5. Testing rotor performance calculations...")
        test_climb_velocities = [0, 2, 4, 6, 8]
        
        for climb_vel in test_climb_velocities:
            thrust = rotor.total_thrust(climb_vel, n_divisions=30, density=density)
            power = rotor.total_power(climb_vel, n_divisions=30)
            print(f"Climb velocity {climb_vel} m/s: Thrust = {thrust:.1f} N, Power = {power:.1f} W")
        
        print("\n6. Testing hover capability...")
        can_hover, message = helicopter.can_hover(climb_velocity=0)
        print(f"Hover test: {message}")
        
        print("\n7. Mission analysis...")
        mission_results = helicopter.mission_analysis([0, 1, 2, 3, 4, 5])
        print("\nClimb Velocity | Thrust Avail | Thrust Req | Power Req | Can Achieve | Excess Thrust")
        print("-" * 90)
        for result in mission_results:
            print(f"{result['climb_velocity']:11.1f} | {result['thrust_available']:11.1f} | "
                  f"{result['thrust_required']:9.1f} | {result['power_required']:8.1f} | "
                  f"{str(result['can_achieve']):10s} | {result['excess_thrust']:12.1f}")
        
        print("\n8. Testing elemental calculations...")
        r_test = rotor.radius_of_rotors * 0.7  # 70% radius
        lambda_val, F_val = rotor.elemental_inflow_ratio(r_test, climb_velocity=2.0)
        phi = rotor.get_phi_r(r_test, climb_velocity=2.0)
        alpha_eff = rotor.get_alpha_effective_r(r_test, climb_velocity=2.0)
        chord = rotor.get_chord_length(r_test)
        
        print(f"At r = {r_test:.2f} m (70% radius) with 2 m/s climb:")
        print(f"  Inflow ratio (λ): {lambda_val:.4f}")
        print(f"  Tip loss factor (F): {F_val:.4f}")
        print(f"  Flow angle (φ): {np.rad2deg(phi):.2f}°")
        print(f"  Effective AoA: {np.rad2deg(alpha_eff):.2f}°")
        print(f"  Chord length: {chord:.3f} m")
        
        print("\n9. Performance comparison across different densities...")
        densities = [0.8, 1.0, 1.2, 1.4]  # kg/m³
        print("Density | Thrust | Power")
        print("-" * 25)
        for dens in densities:
            thrust = rotor.total_thrust(climb_velocity=0, density=dens)
            power = rotor.total_power(climb_velocity=0)
            print(f"{dens:6.1f} | {thrust:5.1f} | {power:5.1f}")
    ...
    def test_component_isolation():
        ...
        print("\n1. Testing Environment standalone...")
        env = Environment()
        env.set_atmosphere_parameters(288.15, 1e6, 0, 101325, 0.0065, 0, 0)
        temp = env.get_temperature(altitude=2000)
        print(f"Temperature at 2000m: {temp:.2f} K")
        
        print("\n2. Testing Rotor without Environment...")
        rotor = Rotor()
        rotor.set_rotor_parameters(3, 30, 40, "0012", 4.5, 0.3, 6, 0.35, 0.15, 10, -0.06)
        thrust = rotor.total_thrust(climb_velocity=0, density=1.225)
        print(f"Rotor thrust (standard conditions): {thrust:.1f} N")
        
        print("\n3. Testing error handling...")
        try:
            empty_rotor = Rotor()
            empty_rotor.total_thrust(0)
