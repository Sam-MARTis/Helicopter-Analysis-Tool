...
def map_r_ψ_to_mesh_indices_static(
    r, ψ, rc, R, r_divisions, ψ_divisions
) -> Tuple[int, int]:
    if r < rc or r > R:
...
def map_mesh_indices_to_r_ψ_static(
    i, j, rc, R, r_divisions, ψ_divisions
) -> Tuple[float, float]:
...
@njit(parallel=True)
def compute_mesh_secondary_quantities(mesh, chord_value_cache, cosβ0, a, Cd0, Cd_Clsq_slope, ρ, b, 
                                    rc, R, r_divisions, ψ_divisions):
    """
    Njit-compiled function to compute secondary derived quantities for the mesh in parallel.
    """
    dr = (R - rc) / r_divisions
    dψ = 2 * np.pi / ψ_divisions
    
    for i in prange(r_divisions):
        for j in range(ψ_divisions):
            # Get mesh values
            α = mesh[i, j, 1]
            Up = mesh[i, j, 2]
            Ut = mesh[i, j, 3]
            
            # Get chord value for this radial station
            c = chord_value_cache[i]
            
            # Compute flow angle and velocity magnitude squared
            θ = np.arctan2(Up, Ut)
            U_sq = Up * Up + Ut * Ut
            
            # Compute lift and drag coefficients
            Cl = a * (3 * α / (3 + α * α))  # tanh approximation
            Cd = Cd0 + Cd_Clsq_slope * Cl * Cl
            
            # Compute forces per unit area
            dL = 0.5 * ρ * U_sq * c * Cl
            dD = 0.5 * ρ * U_sq * c * Cd
            
            # Compute thrust and drag components
            dT = (dL * np.cos(θ) - dD * np.sin(θ)) * cosβ0
            d_drag = dL * np.sin(θ) + dD * np.cos(θ)
            
            # Store results in mesh (accounting for number of blades)
            mesh[i, j, 4] = dT * b
            mesh[i, j, 5] = d_drag * b
...
class Rotor:
    ...
    def set_all_calculation_batch_properties(self, Thrust_Needed, Vy, Ω, θ0, θ1s, θ1c):
        self.Vy = Vy
        self.set_calculation_batch_properties(Thrust_Needed, Ω, θ0, θ1s, θ1c)
        
        
    def map_r_ψ_to_mesh_indices(self, r, ψ) -> Tuple[int, int]:
        i, j = map_r_ψ_to_mesh_indices_static(
            r, ψ, self.rc, self.R, self.r_divisions, self.ψ_divisions
        )
        return i, j
    ...
    def map_mesh_indices_to_r_ψ(self, i, j) -> Tuple[float, float]:
        ...
        r, ψ = map_mesh_indices_to_r_ψ_static(
            i, j, self.rc, self.R, self.r_divisions, self.ψ_divisions
        )
        return r, ψ
    ...
    def get_I(self, set=True):
        I = (
            (1 / 3)
            * ((self.mass / self.blade_count) / (self.R - self.rc))
            * (np.power(self.R, 3) - np.power(self.rc, 3))
        )
        if set:
            self.I = I
        return I
    ...
    def get_effective_aoa(self, r, ψ, set_u_vels=True):
        ...
        Up = (
            v
            + r * dβ_by_dt
            + self.Vinfty * sin(self.β0) * cos(ψ)
            + self.Vinfty * sin(self.α_tpp)
        )
    ...
    def update_grid_induced_velocity(self):
        for i in range(self.r_divisions):
            for j in range(self.ψ_divisions):
                r, ψ = self.map_mesh_indices_to_r_ψ(i, j)
                v = self.get_v(r, ψ)
                self.mesh[i, j, 0] = v + self.Vy
        
    # def update_grid_induced_velocity_for_climb(self, climb_rate):
    #     for i in range(self.r_divisions):
    #         for j in range(self.ψ_divisions):
    #             r, ψ = self.map_mesh_indices_to_r_ψ(i, j)
    #             v = self.get_v(r, ψ) - climb_rate
    #             self.mesh[i, j, 0] = v
    ...
    def calculate_primary_derived_quantities(
        self, W, D, coning_angle_iterations=5, β0_step_fraction=0.6
    ):
        self.get_Area_Disc()
        self.get_I()
        self.get_Ct()
    ...
    def update_grid_secondary_derived_quantities(self):
        ...
        self.update_grid_effective_aoa_and_U_vels()
        cosβ0 = cos(self.β0)
        r_div = self.r_divisions
        ψ_div = self.ψ_divisions
        chord_value_cache = np.array([self.chord_function(r) for r in np.linspace(self.rc + self.dr / 2, self.R - self.dr / 2, r_div)])
        
        # Call the njit-compiled helper function
        compute_mesh_secondary_quantities(
            self.mesh, chord_value_cache, cosβ0, self.a, self.Cd0, self.Cd_Clsq_slope, 
            self.ρ, self.blade_count, self.rc, self.R, self.r_divisions, self.ψ_divisions
        )
    ...
    def perform_all_calculations(
        ...
        self.calculate_primary_derived_quantities(
            W, D, coning_angle_iterations, β0_step_fraction
        )
...
def trimSolve(
    ...
    if verbose:
        ...
        print(
            f"Initial guess: θ0={θ0/deg_to_rad:.2f}°, θ1s={θ1s/deg_to_rad:.2f}°, θ1c={θ1c/deg_to_rad:.2f}°"
        )

    for iteration in range(iterations):
        rotor.set_calculation_batch_properties(
            Thrust_Needed=Thrust_Needed, Ω=Ω, θ0=θ0, θ1s=θ1s, θ1c=θ1c
        )
        vals_current = rotor.perform_all_calculations(
            W=W,
            D=D,
            coning_angle_iterations=coning_angle_iterations,
            β0_step_fraction=β0_step_fraction,
        )
        ...
        if (
            abs(residual_T) < thrust_tolerance
            and abs(residual_Mx) < moment_tolerance
            and abs(residual_Mz) < moment_tolerance
        ):
            if verbose:
        ...
        if verbose and (iteration % 5 == 0 or iteration < 3):
            print(
                f"Iter {iteration}: T={T_current:.0f}N (err:{residual_T:.0f}), Mx={Mx_current:.0f}N⋅m, Mz={Mz_current:.0f}N⋅m"
            )
        J = np.zeros((3, 3))

        rotor.set_calculation_batch_properties(
            Thrust_Needed=Thrust_Needed, Ω=Ω, θ0=θ0 + θ0_ϵ, θ1s=θ1s, θ1c=θ1c
        )
        vals_θ0 = rotor.perform_all_calculations(
            W=W,
            D=D,
            coning_angle_iterations=coning_angle_iterations,
            β0_step_fraction=β0_step_fraction,
        ...
        rotor.set_calculation_batch_properties(
            Thrust_Needed=Thrust_Needed, Ω=Ω, θ0=θ0, θ1s=θ1s + θ1s_ϵ, θ1c=θ1c
        )
        vals_θ1s = rotor.perform_all_calculations(
            W=W,
            D=D,
            coning_angle_iterations=coning_angle_iterations,
            β0_step_fraction=β0_step_fraction,
        ...
        rotor.set_calculation_batch_properties(
            Thrust_Needed=Thrust_Needed, Ω=Ω, θ0=θ0, θ1s=θ1s, θ1c=θ1c + θ1c_ϵ
        )
        vals_θ1c = rotor.perform_all_calculations(
            W=W,
            D=D,
            coning_angle_iterations=coning_angle_iterations,
            β0_step_fraction=β0_step_fraction,
        ...
        if verbose and (iteration % 5 == 0 or iteration < 3):
            print(
                f"  Δθ: [{Δθ[0]/deg_to_rad:.3f}°, {Δθ[1]/deg_to_rad:.3f}°, {Δθ[2]/deg_to_rad:.3f}°]"
            )
            print(
                f"  New θ: [{θ0/deg_to_rad:.3f}°, {θ1s/deg_to_rad:.3f}°, {θ1c/deg_to_rad:.3f}°]"
            )

    rotor.set_calculation_batch_properties(
        Thrust_Needed=Thrust_Needed, Ω=Ω, θ0=θ0, θ1s=θ1s, θ1c=θ1c
    )
    vals_final = rotor.perform_all_calculations(
        W=W,
        D=D,
        coning_angle_iterations=coning_angle_iterations,
        β0_step_fraction=β0_step_fraction,
    ...
    if verbose:
        print(f"\nFinal Results:")
        print(
            f"Controls: θ0={θ0/deg_to_rad:.3f}°, θ1s={θ1s/deg_to_rad:.3f}°, θ1c={θ1c/deg_to_rad:.3f}°"
        )
        print(
            f"Thrust: {vals_final[0]:.0f} N (target: {Thrust_Needed} N, error: {vals_final[0]-Thrust_Needed:.0f} N)"
        )
        print(
            f"Moments: Mx={vals_final[1][0]:.0f} N⋅m, My={vals_final[1][1]:.0f} N⋅m, Mz={vals_final[1][2]:.0f} N⋅m"
        )
...
def print_trim_values_and_state(rotor: Rotor, trim_values, state_values):
    ...
    print(
        f"  Moments: Mx={state_values[1][0]:.0f} N⋅m, My={state_values[1][1]:.0f} N⋅m, Mz={state_values[1][2]:.0f} N⋅m"
    )
...
if __name__ == "__main__":
    ...
    trim_vals, trim_outs = trimSolve(
        rotor=rotor,
        Ω=Ω,
        θ0_initial=10.015 * deg_to_rad,
        θ1s_initial=-14.02 * deg_to_rad,
        θ1c_initial=3 * deg_to_rad,
        W=W,
        D=D,
        coning_angle_iterations=5,
        β0_step_fraction=1,
        iterations=10,
        relaxation=1,
        verbose=False,
