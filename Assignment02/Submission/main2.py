...
rotor1 = Rotor(rotor_mass=150, blade_count=3, R=5, rc=0.2, chord_function=lambda r: 0.3, θtw=θtw, ρ=1.225)
...
def _calculate_speed_performance(args):
    ...
    try:
        ...
        if trim_power > 0:
            return V, trim_power, V/trim_power  
        else:
            return V, None, None
...
class MissionPlanner:
    ...
    def get_max_range_speed_mt(self, weight, dV=30, V_start=10, V_end=150, num_workers=None) -> Tuple[float, float]:
        ...
        if num_workers is None:
            num_workers = min(mp.cpu_count(), MAX_CPU_WORKERS)  
    ...
    def get_trim_conditions(self, V, iterations = 20, relaxation = 0.6):
        ...
        trim_vals, trim_state_vals = trimSolve(rotor=self.rotor, Thrust_Needed=T_Needed, Ω=self.Ω, θ0_initial=5.015*deg_to_rad, θ1s_initial=-4.02*deg_to_rad, θ1c_initial=3*deg_to_rad, W=(self.dryweight + self.fuelweight)*g, D=D, coning_angle_iterations=2, β0_step_fraction=1.00, iterations=iterations, relaxation=relaxation, verbose=False)
...
def test_max_speed_power_constraint(vmin=10, vmax=250, dv=10):
    rotor = Rotor(rotor_mass=150, blade_count=3, R=12, rc=0.4, chord_function=lambda r: 0.5, θtw=θtw, ρ=RHO)  
    for v in range(vmin, vmax+1, dv):
        ...
        try:
            ...
            trim_power = trim_state_vals[2]
            print(f"V={v} m/s: Power={trim_power/10000:.1f} kW")
...
def test_max_speed_stall_constraint(vmin=110, vmax=250, dv=10):
    stall_angle = 20*deg_to_rad
    rotor = Rotor(rotor_mass=150, blade_count=3, R=12, rc=0.4, chord_function=lambda r: 0.5, θtw=θtw, ρ=RHO)  
    for v in range(vmin, vmax+1, dv):
        D = get_Parasitic_Drag(ρ=1.0, f=0.4, Vinfty=v)
        T_Needed = get_Thrust_Total(W=(MASS_OF_CHINOOK/NUMBER_OF_MAIN_ROTORS)*g, D=D)
        rotor.set_calculation_batch_properties(Thrust_Needed=T_Needed, Ω=Ω, θ0=5.015*deg_to_rad, θ1s=-4.02*deg_to_rad, θ1c=3*deg_to_rad)
        try:
            trims, trim_state_vals = trimSolve(rotor=rotor, Thrust_Needed=T_Needed, Ω=Ω, θ0_initial=5.015*deg_to_rad, θ1s_initial=-4.02*deg_to_rad, θ1c_initial=3*deg_to_rad, W=(MASS_OF_CHINOOK/NUMBER_OF_MAIN_ROTORS)*g, D=D, coning_angle_iterations=2, β0_step_fraction=1.00, iterations=10, relaxation=0.3, verbose=False)
...
if __name__ == "__main__":
    ...
    rotor = Rotor(rotor_mass=150, blade_count=3, R=5, rc=0.4, chord_function=lambda r: 0.5, θtw=θtw, ρ=RHO)  
    ...
    drag = get_Parasitic_Drag(ρ=1.0, f=0.4, Vinfty=111)
    thrust_needed = np.sqrt(((MASS_OF_CHINOOK/NUMBER_OF_MAIN_ROTORS)*g)**2 + drag**2)
    trims, trim_out = trimSolve(rotor=rotor, Thrust_Needed=thrust_needed,  Ω=Ω, θ0_initial=5.015*deg_to_rad, θ1s_initial=-4.02*deg_to_rad, θ1c_initial=3*deg_to_rad, W=(MASS_OF_CHINOOK/NUMBER_OF_MAIN_ROTORS)*g, D=drag, coning_angle_iterations=3, β0_step_fraction=1.00, iterations=20, relaxation=0.8, verbose=True)
    print("Θ0 (deg):", trims[0]/deg_to_rad)
    print("Θ1s (deg):", trims[1]/deg_to_rad)
    print("Θ1c (deg):", trims[2]/deg_to_rad)
    print("Θtw (deg):", rotor.θtw/deg_to_rad)
    print("α_tpp (deg):", rotor.α_tpp/deg_to_rad)
    ...
    print("Fy (N): ", trim_out[0]*cos(rotor.α_tpp) - MASS_OF_CHINOOK*g/NUMBER_OF_MAIN_ROTORS)
    print("Fz (N): ", 0)
    print("Coning angle (deg):", rotor.β0/deg_to_rad)
    
    
    test_max_speed_power_constraint()
    test_max_speed_stall_constraint()
